...
const plans = {
  free: {
    label: "Free",
    summary: "3 stories/month, up to 10 min",
    monthlyStories: 3,
    maxDuration: 10,
    canSave: false,
    canUseAudio: false,
  ...
  premier: {
    label: "Premier",
    summary: "15 stories/month, saves, up to 30 min",
    monthlyStories: 15,
    maxDuration: 30,
    canSave: true,
    canUseAudio: false,
    savedLimit: 15,
    note: "Premier package: 15 stories/month, saved story library, and stories up to 30 minutes.",
  },
  plus: {
    label: "DreamScapes Plus",
    summary: "30 stories/month, saves + audio, up to 1 hour",
    monthlyStories: 30,
    maxDuration: 60,
    canSave: true,
    canUseAudio: true,
    savedLimit: 30,
    note: "DreamScapes Plus: 30 stories/month, larger saved library, audio narration, and stories up to 1 hour.",
  },
};
...
document.querySelectorAll("[data-plan-select]").forEach((button) => {
  button.addEventListener("click", () => {
    ...
    upgradeNote.textContent = `${plan.label} is now the active subscription for this prototype.`;
    showScreen("builder");
  });
});
